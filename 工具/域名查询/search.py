...
class ns_Thread(threading.Thread):
        def __init__(self,domainname,q):
            threading.Thread.__init__(self)
            self.e = q
            self.domainname = domainname
            self.nsinformdictlist = []
        def run(self):
...
def get_register_country(domain):
    try:
        ...
        print("err in get register")
    try:
        register_country = register_dict.get(register)
        if register_country == None:
            register_country ="CN"
    except:
...
def get_dns_and_layer(domain_name):
    ns_inform_list = []
    ...
    try:
        ...
        for i in ns1.response.answer:
            for j in i.items:
                ...
                ns1_mid_domain_name = get_mid_of_domain_name(j)
                if ns1_mid_domain_name not in name_list_1:
                    name_list_1.append(ns1_mid_domain_name)
                    list1 = []
                        ...
                        for p in ns2.response.answer:
                            for q in p.items:
                                ...
                                if ((get_mid_of_domain_name(q) != get_mid_of_domain_name(j)) and (
                                    ...
                                    ns2_mid_domain_name = get_mid_of_domain_name(q)
                                    if ns2_mid_domain_name not in name_list_2:
                                        name_list_2.append(ns2_mid_domain_name)
                                        ...
                                        for m in ns3.response.answer:
                                            for n in m.items:
                                                ...
                                                if (get_mid_of_domain_name(n) != get_mid_of_domain_name(q) and (
                                                    ...
                                                    j)):
                                                    ns3_mid_domian_name = get_mid_of_domain_name(n)
                                                    if(ns3_mid_domian_name not in name_list_3):
                                                        ...
                                                        for a in ns4.response.answer:
                                                            for b in a.items:
                                                                ...
                                                                if (get_mid_of_domain_name(b) != get_mid_of_domain_name(
                                                                        n) and get_mid_of_domain_name(
                                                                    b) != get_mid_of_domain_name(
                                                                    q) and (
                                                                        get_mid_of_domain_name(b) != get_mid_of_domain_name(
                                                                    domain_name)) and get_mid_of_domain_name(
                                                                    b) != get_mid_of_domain_name(j)):
                                                                    ns4_mid_domain_name = get_mid_of_domain_name(b)
                                                                    if ns4_mid_domain_name not in name_list_4:
                                                                        ...
                                                                        for x in ns5.response.answer:
                                                                            for y in x.items:
                                                                                y = str(y)
                                                                                y = y[:-1]
                                                                                if (get_mid_of_domain_name(
                                                                                        y) != get_mid_of_domain_name(
                                                                                        b)):
                                                                                    list5 = []
                                                                                    list5.append(b)
                                                                                    list5.append(y)
...
def get_cdns(domain):
    ...
    ret = requests.get(url=url, headers=headers, timeout=300, verify=False)
    page_text = ret.text
    tree = etree.HTML(page_text)
    div_list = tree.xpath('/html/body/div[1]/div/div/div')
    i = 1
    CDN_list = []
    for div in div_list:
        if i == 1:
            ...
            try:
                location = div.xpath('./div[1]/text()')[0]
                provider = div.xpath('./div[2]/text()')[0]
                if (location == 'China' and provider != '-'):
                    CDN_list.append(provider)
            except:
                pass
    cdncountry = []
    if len(CDN_list) != 0:
        for i in CDN_list:  # 对列表中的每个CDN服务商，在字典中寻找其国籍，并依次存入cdncountry列表
            if CDNdict.get(i) != None:
...
def whois_search_of_one_domain(domain):
    key = "69915ef10de44c419fb594eacb6f5b8a"
    res = requests.get("https://apidatav2.chinaz.com/single/whois?key="+key+"&domain="+domain)
...
def run_one(domain,type):
    domain = get_standerd_domain_name(domain)
    ...
    if m == 0:
        ...
        cdn_dict = {}
        ns_dict_list = []
        subthread_1 = ns_Thread(domain,c)
        subthread_1.start()
        ...
        subthread.join()
        subthread_1.join()
        ns_dict_list = c.get()   # 获取全部ns信息，为一个列表，里面单元为字典
        cdn_dict = q.get()
        ...
        new_register_country = get_register_country(domain)
        final_inform.update({"注册商所属国家":new_register_country})
    ...
    register = get_register(domain)
    if register == None:
        register = whois_search_of_one_domain(domain)
    ...
    final_dict = {"domainName":domain,"register":register,"IP":IP,"cname":cname,
                  "NSServer":nsserver,"CDNSercer":cdnserver,"visit_count":visit_count,
...
def run_one_listuse(canshu_list):
    ...
    if m == 0:
        ...
        new_register_country = get_register_country(domain)
        cdn_dict = get_cdns(domain)
        final_inform.update({"注册商所属国家": new_register_country,"CDN服务商信息":cdn_dict})
...
if __name__ == '__main__':
    ...
    {'domainName': 'www.sohu.com', 'register:xxxxx'IP': '221.179.177.18', 'cname': 'gs.a.sohu.com', 'NSServer': 'sohu.com',
